...
* 训练：离线跑 `nlu/train.py`（可用托管 venv，只有 numpy 依赖）
* 推理：`nlu/intent.py` 加载导出的 ``model.npz``，毫秒级、零框架依赖
...
class IntentNet:
    ...
    def __init__(self, vocab_size, n_class, emb=32, hidden=64, seed=20260911):
        ...
        self.labels = list(taxonomy.KEYS)
        # Adam 状态
        self._m = {}
        self._v = {}
        self._t = 0
    ...
    def load(cls, path):
        ...
        net.labels = list(meta.get("labels") or taxonomy.KEYS)
        return net
